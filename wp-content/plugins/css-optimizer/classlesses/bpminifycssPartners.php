<?php
/*
Classlessly add a "more tools" tab 
*/

add_action('admin_init', 'ao_partner_tabs_preinit');
function ao_partner_tabs_preinit() {
    if (apply_filters('bpminifycss_filter_show_partner_tabs',true)) {
        add_filter('bpminifycss_filter_settingsscreen_tabs','ao_add_partner_tabs');
    }
}

function ao_add_partner_tabs($in) {
    $in=array_merge($in,array('ao_partners' => __('Optimize More!','bpminifycss')));
    return $in;
}
...
function ao_partners() {
    ...
    <div class="wrap">
        <h1><?php _e('bpminifycss Settings','bpminifycss'); ?></h1>
        <?php echo bpminifycssConfig::ao_admin_tabs(); ?>
        <?php
            _e("<h2>These bpminifycss power-ups and related services will improve your site's performance even more!</h2>","bpminifycss");
        ?>
        <div>
...
function getAOPartnerFeed() {
    $noFeedText=__( 'Have a look at <a href="http://optimizingmatters.com/">optimizingmatters.com</a> for bpminifycss power-ups!', 'bpminifycss' );
