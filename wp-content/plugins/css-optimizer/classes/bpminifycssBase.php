...
abstract class bpminifycssBase {
    protected $content = '';
    ...
    protected function getpath($url) {
        $url=apply_filters( 'bpminifycss_filter_cssjs_alter_url', $url);
        
        if (strpos($url,'%')!==false) {
            $url=urldecode($url);
        }

        $siteHost=parse_url(bpminifycss_WP_SITE_URL,PHP_URL_HOST);
        ...
        if (strpos($url,'//')===0) {
            ...
            if (bpminifycss_WP_SITE_URL === $siteHost) {
                $url = bpminifycss_WP_SITE_URL.$url;
            } else {
                $subdir_levels=substr_count(preg_replace("/https?:\/\//","",bpminifycss_WP_SITE_URL),"/");
                $url = bpminifycss_WP_SITE_URL.str_repeat("/..",$subdir_levels).$url;
            }
        }
        ...
        if ($thisHost !== $siteHost) {
            ...
            * then apply own filter bpminifycss_filter_cssjs_multidomain takes an array of hostnames
            ...
            */
            ...
            $multidomains = apply_filters('bpminifycss_filter_cssjs_multidomain', $multidomains);
            
            if (!empty($multidomains)) {
                if (in_array($thisHost,$multidomains)) {
                    $url=str_replace($thisHost, parse_url(bpminifycss_WP_SITE_URL,PHP_URL_HOST), $url);
                } else {
                    return false;
                }
            } else {
        ...
        $tmp_ao_root = preg_replace('/https?:/','',bpminifycss_WP_ROOT_URL);
    ...
    protected function url_replace_cdn( $url ) {
        ...
        $cdn_url = apply_filters( 'bpminifycss_filter_base_cdnurl', $this->cdn_url );
        ...
        if ( !empty($cdn_url) )  {
            ...
            if ( ( substr( $url, 0, 1 ) === '/' ) && ( substr( $url, 1, 1 ) !== '/' ) ) {
                ...
                $WPSiteBreakdown = parse_url( bpminifycss_WP_SITE_URL );
        ...
        $url = apply_filters( 'bpminifycss_filter_base_replace_cdn', $url );
        return $url;
    }
    ...
    protected function inject_in_html($payload,$replaceTag) {
        if (strpos($this->content,$replaceTag[0])!== false) {
            ...
            if (!$this->tagWarning) {
                $this->content .= "<!--noptimize--><!-- bpminifycss found a problem with the HTML in your Theme, tag \"".str_replace(array("<",">"),"",$replaceTag[0])."\" missing --><!--/noptimize-->";
            ...
            $out = preg_replace_callback(
                ...
                create_function(
                    ...
                    if (substr($filepath,-3,3)===".js") {
                        ...
                        if (get_option("bpminifycss_js_trycatch")==="on") {
                            $filecontent="try{".$filecontent."}catch(e){}";
                        }
                    } else if ((substr($filepath,-4,4)===".css")) {
                        $filecontent=bpminifycssStyles::fixurls($filepath,$filecontent);
                    }
